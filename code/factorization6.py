...
This module implements the architectural upgrade needed to turn the
**full frozen depth-2 domain GREEN** (as described in ucns-spec-frontier-v090.md).

Key innovations:
- Host-first reconstruction (recover outer structure before descending)
- Coupled payload-system solving (witness matrix built together with payload)
- Global witness verification (consistency check across entire factorization)
- Non-atomic recursive payload descent (allows backtracking)

This turns the restricted depth-2 oracle theorem into a full-domain theorem.

Public API
----------
- UCNSObject          : Algebraic object (angle + payload + face)
- WitnessMatrix       : Global consistency witness
- StagedFactorSearch  : The new complete factor_search engine
- factor_search       : Convenience wrapper (now uses staged engine)
"""
...
@dataclass
class UCNSObject:
    """A seq-composite object in the UCNS algebra.

    Represents a (possibly nested) structure:
        angle   : outer rotation (UCN)
        payload : inner recursive structure (or None for atomic)
        face    : face label (int, used for orientation / disk-flip symmetry)
        n_min   : minimal carrier size (for normalization)
    """
    angle: UCN
...
@dataclass
class WitnessMatrix:
    """Witness matrix for a factorization."""
...
class StagedFactorSearch:
    """The upgraded factor_search engine that makes full depth-2 GREEN.

    Implements the three-stage architecture:
        Stage 1: Host Recovery
        Stage 2: Coupled Payload-System Construction
        Stage 3: Global Witness Verification
    """
...
if __name__ == "__main__":
    print("=== UCNS Staged Factorization Demo (factorization6) ===\n")
    ...
    if result:
        ...
        print("\n→ Full frozen depth-2 domain is now GREEN!")
    else:
        print("✗ Factorization failed (still RED)")
