...
class UCNSObject:
    ...
    def _disk_flip(obj):
        ...
        flipped = UCNSObject(obj.n_dec, obj.n_min, copy.deepcopy(obj.A_minus), obj.F_minus[:])
...
def left_quotient_payload(target, S, catalogue):
    if S is None:
        return target
...
def factor_search_v06(P, catalogue):
...
def factor_search_v062(P, catalogue):
    """v0.6.2 layered factor search with full host enumeration."""
...
if __name__ == "__main__":
    print("=== UCNS v0.6.2 — Full E10 Enumeration + Quotient Fallback ===")
    S2 = create_S2()
    A = UCNSObject(2, 2, [(Fraction(0), S2), (Fraction(1), UNIT)], [0, 0])
    B = UCNSObject(2, 2, [(Fraction(0), S2), (Fraction(1), UNIT)], [0, 0])
    P = multiply(A, B)
    ...
    print("factor_search_v062 result:", result)
    if isinstance(result, tuple):
        rec_A, rec_B = result
        print("Recovered A == original?", rec_A == A)
        print("Recovered B == original?", rec_B == B)
        print("Soundness check (multiply(rec_A, rec_B) == P):", multiply(rec_A, rec_B) == P)
    else:
        print("Returned status:", result)
    print("\n=== All v0.6.2 layers exercised ===")
    print("Host enumeration + direct payload + quotient fallback + new failure taxonomy ready.")
